...
| 4 | `source-map` | 5 | `heuristic` | `native-enhancement` | `runtime-verified`: `bash lazyworkbuddy-plugin/scripts/lazyworkbuddy-mcp-test.sh` | Searches `reference/lazycodex/` and `docs/`, reads excerpts, lists paths, and hashes files; grep/file-index based, not a semantic code index. |
