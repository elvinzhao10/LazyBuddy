# Lazyworkbuddy Security and Permissions Plan

> v0.1 — Permission boundaries, agent scoping, hook safety, secret redaction

## Overview

Lazyworkbuddy operates within WorkBuddy's permission model. This document defines the **additional** security boundaries we impose: what each agent can and cannot do, which hooks are safe for autonomous execution, how secrets are redacted from evidence, and where manual approval is required.
...
|-------|-----------|-------------|-----------------|------------------|------------------|-------------|
| **Orchestrator** | ✓ | ✓ (only `.lazyworkbuddy/` and plans) | ✓ (only verification) | ✓ | ✗ | ✗ |
| **Planner** | ✓ | ✓ (only `.lazyworkbuddy/plans/`) | ✓ (read-only analysis) | ✓ (explorer, librarian) | ✗ (research only) | ✗ |
| **Explorer** | ✓ | ✗ | ✓ (read-only) | ✗ | ✗ | ✗ |
...
| PostToolUseFailure | Log to state ledger | SAFE | Append-only write to `.lazyworkbuddy/` |
...
| TaskCreated | Append to run ledger | SAFE | Append-only write to `.lazyworkbuddy/` |
| TaskCompleted | Update progress; append ledger | SAFE | Append-only write to `.lazyworkbuddy/` |
...
# 2. Never modify files outside .lazyworkbuddy/
...
| `.lazyworkbuddy/` | All agents (scoped) | Run state, plans, evidence |
| `lazyworkbuddy-plugin/` | Implementer (v0.3+) | Plugin code only |
...
RULES_FILE="${PLUGIN_DATA}/../../.workbuddy/rules/lazyworkbuddy.md"

# Fixed status message — never includes user input
echo "(Lazyworkbuddy): Loading Project Rules"
