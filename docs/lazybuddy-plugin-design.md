...
The LazyBuddy plugin (`lazybuddy-plugin/`) is the installable WorkBuddy plugin that provides the LazyCodex agent harness. It packages Skills, Commands, Agents, Hooks, MCP servers, and verification scripts into a single installable unit.
...
│   ├── lazybuddy-orchestrator.md # Root orchestrator (Sisyphus)
│   ├── lazybuddy-planner.md      # Planning agent (Prometheus)
│   ├── lazybuddy-explorer.md     # Codebase exploration agent
│   ├── lazybuddy-implementer.md  # Implementation worker
│   ├── lazybuddy-verifier.md     # Evidence verification (Oracle)
│   ├── lazybuddy-reviewer.md     # Multi-angle code review
│   ├── lazybuddy-qa-executor.md  # Hands-on QA execution
│   ├── lazybuddy-gate-reviewer.md # Final gate approval
...
| LazyCodex Field | LazyBuddy Field | Rationale |
|----------------|---------------------|-----------|
| `name: "omo"` | `name: "lazybuddy"` | WorkBuddy-branded; clean-room name |
...
| `interface.displayName: "OMO"` | `interface.displayName: "LazyBuddy"` | WorkBuddy-branded |
...
| # | LazyCodex Skill | LazyBuddy Skill | Status | Notes |
|---|----------------|---------------------|--------|-------|
...
| 26 | — | `librarian/SKILL.md` | New | Memory/index/parity maintenance (LazyBuddy addition) |
| 27 | — | `migration-planner/SKILL.md` | New | Cross-platform migration (LazyBuddy addition) |
...
| 13 | `migration-planner` | Host adaptation planning | LazyBuddy-only | Plan-only |
...
LazyCodex is a single `omo` plugin that packages everything. We follow the same pattern: one `lazybuddy` plugin with all components. This keeps install simple and ensures components are versioned together.
...
LazyCodex's omo plugin uses purple (`#7C3AED`) as its brand color. We keep this for brand continuity — LazyBuddy is visually recognizable as "LazyCodex reborn inside WorkBuddy."
