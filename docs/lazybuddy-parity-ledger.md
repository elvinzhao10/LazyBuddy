...
| LazyCodex Method | LazyCodex Source | LazyBuddy Implementation | Status | Notes |
|-----------------|------------------|------------------------------|--------|-------|
...
| Verified loop (`$ulw-loop`) | [ulw-loop/SKILL.md](../dev/reference/lazycodex/plugins/omo/skills/ulw-loop/SKILL.md) | `/lazy-ulw-loop` command + run ledger + hooks | **adapted** | Goal-evidence loop preserved; `.lazybuddy/` replaces `.omo/` |
...
| Boulder progress | `.omo/boulder.json` | `.lazybuddy/runs/<run_id>/state.json` | **adapted** | Schema expanded with checkpoints and explicit progress tracking |
| Evidence ledger | `.omo/start-work/ledger.jsonl` | `.lazybuddy/runs/<run_id>/events.jsonl` | **adapted** | Format preserved; additional event types added |
...
| LazyCodex Role | LazyCodex Source | LazyBuddy Agent | Status | Notes |
|---------------|------------------|---------------------|--------|-------|
...
| LazyCodex Hook | LazyCodex Source | LazyBuddy Hook | Status | Notes |
|---------------|------------------|--------------------|--------|-------|
...
| `status-dashboard` MCP | Aggregate run/task/verification/parity status | **added** | `native-enhancement`; `state-only`; initialize/tools-list `runtime-verified` by `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` (transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`) |
| `context-graph` MCP | Blast-radius and symbol scans | **adapted** | `host-substitution` for LazyCodex `codegraph`; `heuristic-substitute`, not full semantic codegraph parity; `runtime-verified` by `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` (transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`) |
...
| LazyCodex MCP | LazyBuddy Surface | Parity Class | Capability Label | Evidence / Caveat |
|---|---|---|---|---|
| `codegraph` | `context-graph` | `host-substitution` | `heuristic` | `runtime-verified` smoke via `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` (transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`); no semantic call graph parity. |
| `lsp` | `code-intel` | `host-substitution` | `project-tool-backed` for diagnostics/typecheck; `heuristic` for symbols/references/goto | `runtime-verified` smoke via `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` (transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`); no real LSP daemon, workspace rename, or semantic goto-def parity. |
| `context7` | `docs` | `host-substitution` | `heuristic` | `runtime-verified` smoke via `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` (transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`); registry README fetch, not curated Context7. |
...
| none | `run-ledger`, `parity`, `verification`, `source-map`, `status-dashboard` | `native-enhancement` | Mixed `project-tool-backed`, `state-only`, `heuristic` | LazyBuddy-only surfaces; do not count as LazyCodex reference parity. |
...
The plugin scaffold (`lazybuddy-plugin/`) is now structurally complete:

- `.codebuddy-plugin/plugin.json` — valid manifest with all required fields (name, version, skills, commands, agents, hooks, mcpServers, interface)
...
- `init-deep` — hierarchical project memory generation (workbuddy.md + `.lazybuddy/context/`)
...
12 agent role definitions created in `lazybuddy-plugin/agents/`:

**LazyCodex-mapped agents (8):**
- `lazybuddy-orchestrator.md` — Sisyphus orchestrator (source: start-work/SKILL.md)
- `lazybuddy-planner.md` — Prometheus planner (source: ulw-plan/SKILL.md)
- `lazybuddy-explorer.md` — Codebase explorer (source: explorer.toml)
- `lazybuddy-implementer.md` — Implementation executor (source: lazycodex-executor.toml)
- `lazybuddy-verifier.md` — Oracle verifier (source: lazycodex-gate-reviewer.toml)
- `lazybuddy-reviewer.md` — Momus+Metis reviewer (source: momus.toml + metis.toml)
- `lazybuddy-qa-executor.md` — QA executor (source: lazycodex-qa-executor.toml)
- `lazybuddy-gate-reviewer.md` — Final gate Oracle (source: lazycodex-gate-reviewer.toml)

**WorkBuddy-native agents (4):**
- `lazybuddy-librarian.md` — Memory maintenance (source: librarian.toml)
- `lazybuddy-migration-planner.md` — Host-adapter planning (LazyBuddy innovation)
- `lazybuddy-context-indexer.md` — Repo structure indexing (source: init-deep skill Phase 1)
- `lazybuddy-security-auditor.md` — Security review (source: review-work Agent 4)
...
- **.lazybuddy/ directory:** context/, runs/<run_id>/{state.json, events.jsonl, plan.md, checkpoints/, evidence/, verification/, review/, agent_outputs/, artifacts/, memory_updates/, final_report.md}
...
- **source-map** (5 tools): reads dev/reference/lazycodex/ + .lazybuddy/context/ — index_repo, search_method_evidence, read_evidence_excerpt, list_source_paths, compute_file_hash
...
The MCP runtime currently declares 8 servers in `lazybuddy-plugin/.mcp.json`: `run-ledger`, `parity`, `verification`, `source-map`, `status-dashboard`, `context-graph`, `code-intel`, and `docs`.
...
- `state-only`: LazyBuddy state/docs access only.

Runtime evidence: `bash lazybuddy-plugin/scripts/lazybuddy-mcp-test.sh` exercised initialize/tools-list for all 8 servers and representative safe tool calls for `run-ledger`, `parity`, `source-map`, `context-graph`, `code-intel`, and `docs`. Transcript: `.omo/evidence/task-4-diagnosis-v0-12-lazybuddy.txt`.
