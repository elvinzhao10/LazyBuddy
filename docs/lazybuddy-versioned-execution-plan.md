...
| CREATE | `.workbuddy/rules/lazybuddy.md` | Project rules that guide agent behavior |
...
3. Create `lazybuddy.md` rules: LazyCodex-derived conventions, non-negotiables, verification discipline
...
2. `rules/lazybuddy.md` lists at least 5 enforceable rules
...
- `rules/lazybuddy.md` is specific to this project (not generic advice)
...
### Objective
Create the installable `lazybuddy-plugin/` shell with a valid `.codebuddy-plugin/plugin.json` manifest, empty component directories, and a working install/uninstall story.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/.codebuddy-plugin/plugin.json` | Plugin manifest |
| CREATE | `lazybuddy-plugin/skills/` | Skills directory (empty for now) |
| CREATE | `lazybuddy-plugin/commands/` | Commands directory (empty for now) |
| CREATE | `lazybuddy-plugin/agents/` | Agents directory (empty for now) |
| CREATE | `lazybuddy-plugin/hooks/hooks.json` | Empty hooks scaffold |
| CREATE | `lazybuddy-plugin/scripts/` | Scripts directory |
| CREATE | `lazybuddy-plugin/README.md` | Plugin documentation |
...
### Rollback Strategy
Delete the `lazybuddy-plugin/` directory. No other files affected.
...
- **Risk:** `lazybuddy-plugin/.workbuddy-plugin/plugin.json` vs `.codebuddy-plugin/plugin.json` naming. **Mitigation:** Use `.codebuddy-plugin/plugin.json` as documented compat fallback.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/skills/init-deep/SKILL.md` | Deep project initialization |
| CREATE | `lazybuddy-plugin/skills/ulw-plan/SKILL.md` | Prometheus planning |
| CREATE | `lazybuddy-plugin/skills/start-work/SKILL.md` | Orchestrated execution |
| CREATE | `lazybuddy-plugin/skills/ulw-loop/SKILL.md` | Verified completion loop |
| CREATE | `lazybuddy-plugin/skills/ultrawork/SKILL.md` | Binding ultrawork directive |
| CREATE | `lazybuddy-plugin/skills/review-work/SKILL.md` | 5-agent parallel review |
| CREATE | `lazybuddy-plugin/skills/programming/SKILL.md` | Strict coding discipline |
| CREATE | `lazybuddy-plugin/skills/remove-ai-slops/SKILL.md` | AI-looking code cleanup |
| CREATE | `lazybuddy-plugin/skills/git-master/SKILL.md` | Git workflow discipline |
| CREATE | `lazybuddy-plugin/skills/debugging/SKILL.md` | Systematic debugging |
| CREATE | `lazybuddy-plugin/commands/init-deep.md` | Command wrapper |
| CREATE | `lazybuddy-plugin/commands/ulw-plan.md` | Command wrapper |
| CREATE | `lazybuddy-plugin/commands/start-work.md` | Command wrapper |
| CREATE | `lazybuddy-plugin/commands/ulw-loop.md` | Command wrapper |
| CREATE | `lazybuddy-plugin/commands/review-work.md` | Command wrapper |
...
3. Replace `.omo/` path references with `.lazybuddy/` equivalents
...
- `.omo/` paths replaced with `.lazybuddy/`
- Each Skill loads without errors

### Rollback Strategy
Delete created skill and command files from `lazybuddy-plugin/`.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/agents/planner.md` | Planning subagent (Prometheus) |
| CREATE | `lazybuddy-plugin/agents/explorer.md` | Codebase exploration |
| CREATE | `lazybuddy-plugin/agents/implementer.md` | Implementation worker |
| CREATE | `lazybuddy-plugin/agents/verifier.md` | Evidence verification (Oracle) |
| CREATE | `lazybuddy-plugin/agents/reviewer.md` | Multi-angle code review |
| CREATE | `lazybuddy-plugin/agents/librarian.md` | Memory/index/parity maintenance |
| CREATE | `lazybuddy-plugin/agents/qa-executor.md` | Hands-on QA execution |
| CREATE | `lazybuddy-plugin/agents/gate-reviewer.md` | Final gate approval |
| CREATE | `lazybuddy-plugin/agents/orchestrator.md` | Root orchestrator (Sisyphus) |
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/hooks/hooks.json` | Complete hooks configuration |
| CREATE | `lazybuddy-plugin/scripts/hook-session-start.sh` | SessionStart: load rules, check bootstrap |
| CREATE | `lazybuddy-plugin/scripts/hook-user-prompt-submit.sh` | UserPromptSubmit: ultrawork/loop detection |
| CREATE | `lazybuddy-plugin/scripts/hook-pre-tool-use.sh` | PreToolUse: budget enforcement |
| CREATE | `lazybuddy-plugin/scripts/hook-post-tool-use.sh` | PostToolUse: diagnostics, rule matching |
| CREATE | `lazybuddy-plugin/scripts/hook-post-tool-use-failure.sh` | PostToolUseFailure: ledger logging |
| CREATE | `lazybuddy-plugin/scripts/hook-pre-compact.sh` | PreCompact: cache reset, state preservation |
| CREATE | `lazybuddy-plugin/scripts/hook-stop.sh` | Stop: continuation check |
| CREATE | `lazybuddy-plugin/scripts/hook-stop-failure.sh` | StopFailure: recovery attempt |
| CREATE | `lazybuddy-plugin/scripts/hook-subagent-stop.sh` | SubagentStop: evidence verify + continuation |
| CREATE | `lazybuddy-plugin/scripts/hook-task-created.sh` | TaskCreated: ledger entry |
| CREATE | `lazybuddy-plugin/scripts/hook-task-completed.sh` | TaskCompleted: progress update |
...
8. Stop/SubagentStop: read `.lazybuddy/runs/<run_id>/state.json`; if unchecked work, signal continuation
...
6. All hooks have status messages following LazyCodex convention: `(LazyBuddy): ...`
...
### Objective
Implement the `.lazybuddy/runs/<run_id>/` schema: state.json, events.jsonl, checkpoints, and the loop protocol that enables resumable autonomous runs.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/scripts/ledger-init.sh` | Initialize run state |
| CREATE | `lazybuddy-plugin/scripts/ledger-append.sh` | Append event to ledger |
| CREATE | `lazybuddy-plugin/scripts/ledger-checkpoint.sh` | Create checkpoint snapshot |
| CREATE | `lazybuddy-plugin/scripts/ledger-restore.sh` | Restore from checkpoint |
| CREATE | `lazybuddy-plugin/scripts/loop-monitor.sh` | Loop health monitoring |
| CREATE | `docs/lazybuddy-state-ledger-design.md` | Schema documentation |
| MODIFY | `lazybuddy-plugin/skills/start-work/SKILL.md` | Update Phase 2/4/5 for .lazybuddy/ paths |
| MODIFY | `lazybuddy-plugin/skills/ulw-loop/SKILL.md` | Update for .lazybuddy/ state |
...
### Rollback Strategy
Since state files are under `.lazybuddy/` (not committed), errors do not affect repo. Delete corrupted state and restart run.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/.mcp.json` | MCP server configuration |
| CREATE | `lazybuddy-plugin/mcp/run-ledger/server.js` | MCP: query/append run state |
| CREATE | `lazybuddy-plugin/mcp/verification/server.js` | MCP: run verification checks |
| CREATE | `lazybuddy-plugin/mcp/parity-dashboard/server.js` | MCP: parity tracking (optional) |
| CREATE | `lazybuddy-plugin/scripts/parity-check.sh` | CLI: compare LazyBuddy vs LazyCodex behavior |
| MODIFY | `lazybuddy-plugin/.codebuddy-plugin/plugin.json` | Add `mcpServers` field |
...
5. Create optional `parity-dashboard` MCP: compare LazyBuddy behavior against LazyCodex source
...
| Action | File | Purpose |
|--------|------|---------|
| MODIFY | `lazybuddy-plugin/agents/verifier.md` | Strengthen evidence checking rules |
| MODIFY | `lazybuddy-plugin/agents/reviewer.md` | Add consistency checks across 5 lanes |
| CREATE | `lazybuddy-plugin/agents/librarian.md` | Memory/index/parity maintenance agent |
| CREATE | `lazybuddy-plugin/skills/librarian/SKILL.md` | Librarian workflow |
| CREATE | `lazybuddy-plugin/scripts/memory-update.sh` | Automated memory update script |
| CREATE | `docs/lazybuddy-known-gaps.md` | Parity gap ledger |
| MODIFY | `lazybuddy-plugin/skills/start-work/SKILL.md` | Update Phase 4 verification gates |
| MODIFY | `lazybuddy-plugin/skills/review-work/SKILL.md` | Harden review lane criteria |
...
3. Hardening — Librarian:
   ...
   - After parity changes: update `docs/lazybuddy-known-gaps.md`
...
### Objective
Create a reusable migration planner that can port LazyCodex semantics to any host platform, and the host adapter templates that make LazyBuddy itself portable.

**LazyCodex source:** No direct equivalent — this is a LazyBuddy innovation that generalizes our adaptation experience.

### Files to Create/Modify
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `lazybuddy-plugin/skills/migration-planner/SKILL.md` | Reusable migration workflow |
| CREATE | `lazybuddy-plugin/agents/migration-planner.md` | Migration planner agent |
...
### Implementation Steps
1. Extract the LazyCodex → LazyBuddy adaptation methodology into a repeatable workflow
...
### Objective
Run LazyBuddy on itself: use `/lazy-ulw-plan` to plan a feature, `/lazy-start-work` to execute it, `/lazy-ulw-loop` to verify completion, and `/lazy-review-work` to review. This is an end-to-end test that exercises every component.

**LazyCodex source:** No direct equivalent — this is a LazyBuddy quality gate.

### Files to Create/Modify
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `.lazybuddy/runs/dogfood-v0.11/state.json` | Dogfood run state (generated) |
| CREATE | `.lazybuddy/runs/dogfood-v0.11/events.jsonl` | Dogfood evidence (generated) |
...
### Rollback Strategy
Dogfood results are under `.lazybuddy/` and `docs/` — no product code affected.
...
| Action | File | Purpose |
|--------|------|---------|
| CREATE | `docs/lazybuddy-parity-report.md` | Final parity comparison |
| CREATE | `docs/lazybuddy-release-notes.md` | Release notes for v0.12 |
| MODIFY | `docs/lazybuddy-known-gaps.md` | Update with all remaining gaps |
| MODIFY | `lazybuddy-plugin/.codebuddy-plugin/plugin.json` | Bump version to 0.12.0 |
| MODIFY | `lazybuddy-plugin/README.md` | Final release documentation |

### Implementation Steps
1. Run full parity check: compare every LazyCodex method against LazyBuddy implementation
...
- [ ] `ORCHESTRATION COMPLETE` for LazyBuddy v0.12
