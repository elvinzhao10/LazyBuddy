...
| R-003 | Hook model mismatch — Codex `Stop/SubagentStop` continuation hook not natively available; replaced by prompt-level polling | **Medium** | Medium | Implement prompt-level re-entry: on each turn, scan run state and re-queue undone work | Manual re-entry via `/lazy-start-work --continue` flag | Verify that re-entry picks up unfinished tasks from previous turn | v0.6 | RESOLVED — prompt-level re-entry works; no hook needed |
