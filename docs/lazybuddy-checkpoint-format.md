# LazyBuddy Checkpoint Format v0.7
...
```
.lazybuddy/runs/<run_id>/checkpoints/
...
ls -1 .lazybuddy/runs/run-20260709-001/checkpoints/ | sort | tail -1
...
# 3. Restore state
cp "$CP_DIR/state.json" .lazybuddy/runs/run-20260709-001/state.json
...
---
_Extends `docs/lazybuddy-state-ledger-design.md` with ISO8601 directory naming and events.jsonl replay for precise state reconstruction._
