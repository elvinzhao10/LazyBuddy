...
- **Consistent:** Lazyworkbuddy commands keep the LazyCodex name where it communicates parity; add WorkBuddy-native aliases where useful (e.g., both `/lazy-review-work` and `/review`)
...
/lazy-init-deep
    │
    ▼
/lazy-ulw-plan ──────────► .lazyworkbuddy/plans/<slug>.md
    │
    ▼
/lazy-start-work ────────► .lazyworkbuddy/runs/<run_id>/state.json
    │                       │
