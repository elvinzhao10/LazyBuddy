...
5. [LazyCodex → Lazyworkbuddy Method Trace](#lazycodex--lazyworkbuddy-method-trace)
6. [Design Decisions](#design-decisions)

## Overview

Lazyworkbuddy is a three-layer system that recreates the LazyCodex/OmO agent harness inside WorkBuddy:

| Layer | Location | Purpose | Lifecycle |
|-------|----------|---------|-----------|
| **Plugin** | `lazyworkbuddy-plugin/` | Reusable, installable behavior (Skills, Agents, Hooks, MCP) | Installed once; active across all workspaces |
...
│  lazyworkbuddy-plugin/                                       │
│  ├── .workbuddy-plugin/plugin.json     ← manifest            │
│  ├── skills/                           ← 25+ Skills          │
...
│  ├── rules/lazyworkbuddy.md            ← project rules        │
│  ├── settings.json                     ← project settings     │
│  ├── skills/                           ← local Skills         │
│  ├── commands/                         ← local commands       │
│  └── agents/                           ← local subagents      │
├─────────────────────────────────────────────────────────────┤
│                  RUN STATE (.lazyworkbuddy/)                  │
│  .lazyworkbuddy/                                              │
│  ├── runs/<run_id>/                                             │
...
### Plugin Components (from lazyworkbuddy-plugin/)
...
| Project rules | `.workbuddy/rules/lazyworkbuddy.md` | `.codex/rules/` — traced to `rules` skill |
...
### Run State Components (from .lazyworkbuddy/)

| Purpose | File | LazyCodex Equivalent |
|---------|------|---------------------|
| Active work state | `.lazyworkbuddy/runs/<run_id>/state.json` | `.omo/boulder.json` — traced to `start-work` Skill Phase 2 |
| Evidence ledger | `.lazyworkbuddy/runs/<run_id>/events.jsonl` | `.omo/start-work/ledger.jsonl` — traced to Phase 4 |
| Checkpoints | `.lazyworkbuddy/runs/<run_id>/checkpoints/` | No direct equivalent (new capability) |
| Plan cache | `.lazyworkbuddy/plans/` | `.omo/plans/` — traced to `ulw-plan` Skill |
| Context index | `.lazyworkbuddy/context/` | Generated by `init-deep` |
...
|-----------------|-------------|------------------------------|
| Plugin packaging | `plugins/omo/.codex-plugin/plugin.json` | `.workbuddy-plugin/plugin.json` |
...
| Boulder progress | `plugins/omo/skills/start-work/SKILL.md` Phase 2 | `.lazyworkbuddy/runs/<run_id>/state.json` |
| Orchestrator delegate | `plugins/omo/skills/start-work/SKILL.md` Phase 3 | WorkBuddy subagent spawning (Agent tool) |
| Evidence ledger | `plugins/omo/skills/start-work/SKILL.md` Phase 4 | `.lazyworkbuddy/runs/<run_id>/events.jsonl` |
...
| MCP tools (5 servers) | `plugins/omo/.mcp.json` | `lazyworkbuddy-plugin/.mcp.json` |
...
### Why `.lazyworkbuddy/` instead of `.omo/`?
