...
| `SubagentStop` | `subagent-stop.sh` | **Critical enforcement.** Verify implementer evidence: extract `EVIDENCE_RECORDED: <path>`, validate path inside `.lazyworkbuddy/`, check file exists + non-empty + not symlink. Max 3 retries. Context pressure passthrough. | [subagent-stop-verifying-lazycodex-executor-evidence.json](../dev/reference/lazycodex/plugins/omo/hooks/subagent-stop-verifying-lazycodex-executor-evidence.json) | 10s |
