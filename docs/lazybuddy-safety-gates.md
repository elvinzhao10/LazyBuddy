...
**Implementation basis:** the `ulw-plan` owner-decision filter for irreversible or destructive work.

## Gate 3: Evidence Verification Gate

**Trigger:** SubagentStop hook — `agent_type_name` matches `lazybuddy-implementer` or `implementer`.

**Logic:**
1. Extract `EVIDENCE_RECORDED: <path>` from `last_assistant_message`
2. Resolve path (relative to cwd if not absolute)
3. Validate: inside `.lazybuddy/` root (`realpath` check)
...
**Implementation basis:** the current executor-evidence verification contract.
...
3. Find active run in `.lazybuddy/runs/` (status: `active` or `paused`)
...
**Implementation basis:** the current continuation and run-state contracts.
...
**Implementation basis:** the current context-pressure handling contract.
...
_See `docs/lazybuddy-hooks.md` for script details and `docs/lazybuddy-hook-test-plan.md` for test procedures._
