# Lazyworkbuddy Agent Orchestration
...
| **Librarian** | Memory files (workbuddy.md, .workbuddy/, docs/) | Rewrites canonical method map, writes to .lazyworkbuddy/ run state |
| **Migration Planner** | Host-adapter plans | Modifies reference/ or product code |
| **Context Indexer** | Repo map, .lazyworkbuddy/context/ | Writes to product paths |
| **Security Auditor** | Secret detection, permission review, unsafe patterns | Writes files |

## State Transitions

The orchestrator tracks run progress in `.lazyworkbuddy/runs/<run_id>/state.json`:
...
_See `docs/lazyworkbuddy-handoff-protocol.md` for inter-agent message formats and `docs/lazyworkbuddy-parallelism-policy.md` for concurrent execution rules._
