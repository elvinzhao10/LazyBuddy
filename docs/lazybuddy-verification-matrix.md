...
Every LazyBuddy workflow must have a verification path: a concrete command, an expected result, and an evidence artifact. This matrix defines the verification strategy for every component.
...
| Plan file created | Check `.lazybuddy/plans/<slug>.md` | File has TL;DR, Todos, Final Verification Wave | Plan file |
...
| Verification Step | Command | Expected Result | Evidence Artifact |
|-------------------|---------|-----------------|-------------------|
| Plan selection works | `/lazy-start-work <slug>` | Selects plan, creates boulder state | `.lazybuddy/runs/<run_id>/state.json` |
...
| Verification Step | Command | Expected Result | Evidence Artifact |
|-------------------|---------|-----------------|-------------------|
| Goal creation works | `/lazy-ulw-loop "fix all type errors"` | Creates goal with success criteria | `.lazybuddy/ulw-loop/` state |
