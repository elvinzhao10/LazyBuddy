...
### v0.7: Run State
`.lazyworkbuddy/` ledger. **Source:** `dev/reference/lazycodex/plugins/omo/.omo/boulder.json` → `state.json`,
...
### v0.8: MCP — 5 servers
`run-ledger`, `parity`, `verification`, `source-map`, `status-dashboard`. **Source:** `dev/reference/lazycodex/plugins/omo/.mcp.json` (pattern). Servers are Lazyworkbuddy-native — not ported from LazyCodex.
