...
def test_revision_marker_changes_when_implementation_changes():
    ...
    assert before == _mock_revision_marker(before_content), \
...
def test_no_new_lineage_evidence_db_transaction_files_introduced():
    ...
    result = subprocess.run(
        ["git", "diff", "--name-only", "v1.0.2..HEAD", "--",
         "**/lineage*", "**/evidence-db*", "**/transaction*"],
...
def test_adaptive_layer_reuses_lazy_verifier_no_parallel_verifier():
    ...
    assert re.search(r"lazy-verifier", combined), \
    ...
    assert not re.search(r"import\s+(lineage|evidence_db|evidence_lineage)", combined), \
...
def test_lazybuddy_verify_script_exists():
    ...
    assert verify_script.exists(), \
        f"lazybuddy-verify.sh must exist at {verify_script}"
...
@pytest.mark.xfail(
    reason="classifier has no prior_snapshot input; stale detection not yet implemented",
    strict=True,
)
def test_stale_prior_snapshot_triggers_reclassification():
    ...
    decision = classify_adaptive_decision("fix the typo in README", {
        ...
        "current_revision_marker": _mock_revision_marker("def new_impl():\n    return 2\n"),
    })
    assert decision["snapshot"]["revisionMarker"] != prior_snapshot["revisionMarker"], \
    ...
    assert any(re.search(r"stale|re-?verif|reclassif", r, re.I) for r in decision["reasons"]), \
...
@pytest.mark.xfail(
    reason="classifier does not emit a re-verification signal when revisionMarker changes",
    strict=True,
)
def test_re_verification_trigger_when_revision_marker_changes():
    ...
    assert re.search(r"re-?verif|stale|revision", joined, re.I), \
