...
FIXTURE_PATH = (Path(__file__).resolve().parent.parent / "contracts" / "fixtures"
                / "v103" / "06-long-horizon-migration.json")
FIXTURE = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))


def _base_run_state():
    return {
        "schema_version": "2", "run_id": "test-w45", "status": "in_progress",
        "objective": "W4.5 continuation", "tasks": [], "tasks_done": 0,
...
def _context_snapshot(current_stage="implement", escalation_count=1, mode="long-horizon"):
    ...
    return {
        ...
        "responsibilities": ["continuity", "exploration", "implementation",
...
@pytest.mark.xfail(
    reason="W4.5 GAP: classify_adaptive_decision does not yet implement "
           "Section 6 step 2 (compatible continuation); currentStage is not "
...
@pytest.mark.xfail(
    reason="W4.5 GAP: classify_adaptive_decision does not yet preserve mode "
           "from a compatible snapshot (Section 6 step 2)."
)
...
@pytest.mark.xfail(
    reason="W4.5 GAP: classify_adaptive_decision does not yet carry over "
           "escalationCount from a compatible snapshot (Section 6 step 2)."
)
...
def test_incompatible_revision_classifier_produces_new_decision():
    ...
    decision = classify_adaptive_decision(request, {"snapshot": snapshot})
    assert isinstance(decision, dict) and isinstance(decision["mode"], str), \
        "classifier must produce a new decision regardless of stale snapshot"
    assert decision["snapshot"]["revisionMarker"] == "git:HEAD", \
...
def test_stale_snapshot_not_used_as_completion_evidence():
    ...
    assert validate_adaptive_snapshot(stale) is True, \
    ...
    assert read.get("completedAt") == "2026-07-20T00:00:00Z", \
...
def test_null_snapshot_in_context_matches_no_snapshot_behavior():
    ...
    decision_without = classify_adaptive_decision(FIXTURE["request"], FIXTURE["context"])
    assert decision_with_null["mode"] == decision_without["mode"], \
        "null snapshot must behave identically to no snapshot"
    assert decision_with_null["snapshot"]["currentStage"] == decision_without["snapshot"]["currentStage"], \
        "null snapshot must produce the same currentStage as no snapshot"
    assert decision_with_null["snapshot"]["escalationCount"] == decision_without["snapshot"]["escalationCount"], \
