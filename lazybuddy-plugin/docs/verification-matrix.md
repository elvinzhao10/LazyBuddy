...
| WorkBuddy sanitized native observations | `bash lazybuddy-plugin/tests/v110-workbuddy-observation-bundle-regression.sh` | All 14 native surfaces remain redacted, receipt/run-ledger linked, observation-only, and non-promoting; Connector identity accepts only adapter-issued versioned redacted references | Node test output plus immutable bundle |
