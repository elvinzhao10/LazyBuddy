...
function fixture() {
  ...
  writeFile(root, 'LazyBuddy/lazybuddy-v1.2.3.tar.gz', 'buddy archive');
  ...
  const candidate = {
    ...
    products: [
      product('lazybuddy', 'LazyBuddy', 'lazybuddy-v1.2.3.tar.gz'),
...
test('advertises a fresh-extraction LazyBuddy verification command', () => {
  ...
  const inventory = [{
    path: 'LazyBuddy/lazybuddy-v1.2.3.tar.gz',
    mode: '0644',
    size: 1,
    sha256: '2'.repeat(64),
  }];
  // When the paired candidate records the product self-verification contract.
  const product = productRecord('lazybuddy', 'LazyBuddy', 'lazybuddy-v1.2.3.tar.gz', SHA, inventory);
