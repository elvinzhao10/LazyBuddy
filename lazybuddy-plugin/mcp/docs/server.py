...
def fetch(url, timeout=20):
    ...
    try:
        r = subprocess.run([CURL, "-sS", "--proto", "=https", "--proto-redir", "=https", "--max-redirs", "0", "--max-time", str(timeout), "-A", "lazybuddy-docs/1.0.2", url],
...
def handle(req, notification):
    ...
    if method == "initialize":
        reply({"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "docs", "version": "1.0.2"}})
        return
