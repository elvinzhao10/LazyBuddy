...
def fetch(url, timeout=20):
    ...
    try:
        r = subprocess.run([CURL, "-sSL", "--max-time", str(timeout), "-A", "lazybuddy-docs/0.15.0-alpha.3", url],
...
def main():
    ...
    if method == "initialize":
        reply({"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}, "serverInfo": {"name": "docs", "version": "0.15.0-alpha.3"}})
        return
