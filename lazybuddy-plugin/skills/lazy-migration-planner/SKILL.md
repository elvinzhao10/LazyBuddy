...
This skill is **read-only for product files**. It writes **only** to `.lazybuddy/adapters/`.
- Allowed: Read, Grep, Glob, Bash (read-only inspection), Write (`.lazybuddy/adapters/` only), Edit (`.lazybuddy/adapters/` only)
...
Write the host-adapter plan to `.lazybuddy/adapters/<target-platform>-<YYYYMMDD>.md`. The plan document must be decision-complete: a downstream worker executing it has ZERO judgment calls about how to translate a source primitive.
...
Append an entry to `.lazybuddy/parity-ledger.jsonl`:

```json
{
  "event": "adapter-plan-created",
  "timestamp": "<ISO 8601>",
  "adapter_plan": ".lazybuddy/adapters/<target-platform>-<YYYYMMDD>.md",
...
4. `.lazybuddy/adapters/<target-platform>-<YYYYMMDD>.md` — the full host-adapter plan
5. `.lazybuddy/parity-ledger.jsonl` — appended migration entry
...
Migration planner complete.
  Adapter plan: .lazybuddy/adapters/<target-platform>-<YYYYMMDD>.md
