...
All review artifacts are written to `.lazybuddy/runs/<run_id>/review/`:
...
Register the review decision in `.lazybuddy/runs/<run_id>/events.jsonl`. If `accept`, hand off to librarian for memory update. If `revise`, hand back to implementer with feedback.
