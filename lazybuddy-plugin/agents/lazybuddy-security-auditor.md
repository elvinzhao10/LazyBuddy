...
Changed files list, full diff, file contents (read directly, not prompt-only), `.lazybuddy/context/commands.json`, dependency manifests (`package.json`, `requirements.txt`, `go.mod`), `.env.example`, `.gitignore`.
