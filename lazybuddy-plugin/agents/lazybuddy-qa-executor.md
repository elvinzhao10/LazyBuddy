...
- Write evidence artifacts under `.lazybuddy/evidence/<goal>/` or the caller's evidence directory only.
...
- **NEVER write outside** `.lazybuddy/evidence/` or the specified evidence directory.
...
1. `.lazybuddy/plans/<plan>.md` — test scenarios, adversarial classes, QA criteria.
2. `.lazybuddy/evidence/` — existing artifacts to avoid duplication.
3. Project-specific run commands from `package.json`, `Makefile`, or `.lazybuddy/context/commands.json`.
...
Every PASS must point to a non-empty artifact. Write artifacts to `.lazybuddy/evidence/<goal>/qa-<timestamp>.json`.
