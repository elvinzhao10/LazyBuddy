...
manage_attempts() {
    ...
    local state_dir="$cwd/.lazybuddy/executor-verify-state"
...
if [ "$AGENT_TYPE" != "lazybuddy-implementer" ] && [ "$AGENT_TYPE" != "implementer" ]; then
    exit 0
fi
...
if [ "$INSIDE" != "yes" ] || [ -z "$REAL_PATH" ]; then
    block_with_retry "EVIDENCE_RECORDED path is outside .lazybuddy/ — rejected."
fi
