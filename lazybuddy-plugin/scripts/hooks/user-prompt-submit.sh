#!/usr/bin/env bash
# user-prompt-submit.sh — UserPromptSubmit hook: detect LazyBuddy command intent + secret warnings.
...
if echo "$MESSAGE" | grep -qE 'sk-[A-Za-z0-9]{20,}|Bearer [A-Za-z0-9._/+=]{20,}|ghp_[A-Za-z0-9]{20,}|gho_[A-Za-z0-9]{20,}|ghu_[A-Za-z0-9]{20,}|ghs_[A-Za-z0-9]{20,}|ghr_[A-Za-z0-9]{20,}'; then
    echo "[LazyBuddy WARNING] Your prompt may contain a secret (API key, token, or credential). Consider redacting it before sending."
fi

# --- Long-horizon work detection without command ---
# Check if user describes multi-step work but didn't use a LazyBuddy command
...
if [ "$HAS_KEYWORD" -eq 1 ]; then
    ...
    if [ "$HAS_CMD" -eq 0 ]; then
        echo "[LazyBuddy TIP] Long-horizon task detected. Consider starting with /lazy-ultrawork for structured planning and execution."
    fi
fi
