...
if [ ! -d "$RUNS_DIR" ]; then
    echo "[LazyBuddy] No active runs found. Run /lazy-start-work to begin a new task."
    exit 0
fi
...
" 2>/dev/null
            ...
            if [ -n "$PLAN" ]; then
                echo "[LazyBuddy] Run /lazy-start-work $PLAN to resume from last checkpoint."
            else
                echo "[LazyBuddy] Run /lazy-start-work to resume from last checkpoint."
            fi
            exit 0
        fi
    fi
