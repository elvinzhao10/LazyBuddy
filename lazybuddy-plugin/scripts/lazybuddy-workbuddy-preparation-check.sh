...
if [ ! -f "$PLUGIN_ROOT/.workbuddy-plugin/plugin.json" ] \
    ...
    printf '%s\n' \
        'ERROR: LazyBuddy plugin root is unavailable; keep this script under the v1.2.1 lazybuddy-plugin/scripts directory.' >&2
    exit 1
fi
...
try:
    ...
    if work_manifest.get("name") != "lazybuddy" or work_manifest.get("version") != version:
        raise ValueError("WorkBuddy manifest must identify lazybuddy version 1.2.1")
    ...
    if entry is None or entry.get("version") != version or entry.get("source") != "./lazybuddy-plugin":
        raise ValueError("release marketplace must contain lazybuddy 1.2.1 from ./lazybuddy-plugin")
