...
function verifyCandidate(candidateInput) {
  ...
  refuse(path.basename(root) !== `live-test-v1.2.1-${combined}`, 'CANDIDATE_NAME_MISMATCH', root);
...
async function assemble(values) {
  ...
  const transaction = { child: null, lock: '', lockOwned: false, staging: path.join(outputRoot, `.live-test-v1.2.1.prestaging-${nonce}`), stagingOwned: true };
  ...
  try {
    ...
    destination = path.join(outputRoot, `live-test-v1.2.1-${manifest.combined_digest}`);
    ...
    const namedStaging = path.join(outputRoot, `.live-test-v1.2.1-${manifest.combined_digest}.staging-${nonce}`);
    ...
    transaction.stagingOwned = false;
    published = true;
    onboardingStaging = path.join(outputRoot, `.live-test-v1.2.1-${manifest.combined_digest}-onboarding.staging-${nonce}`);
