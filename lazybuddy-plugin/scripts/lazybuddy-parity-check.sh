...
WB_PLUGIN_JSON="${PLUGIN_ROOT}/.codebuddy-plugin/plugin.json"
...
# ---- Plugin.json field comparison ----
WB_FIELDS=""
REF_FIELDS=""
MISSING_FIELDS=""

if [ -f "$REF_PLUGIN_JSON" ]; then
    # Extract top-level keys from reference
    # Use python if available, otherwise fall back to grep
    if command -v python3 &>/dev/null; then
        REF_FIELDS=$(python3 -c "import json; keys=list(json.load(open('${REF_PLUGIN_JSON}')).keys()); print(','.join(keys))" 2>/dev/null || echo "")
    fi
fi

if [ -f "$WB_PLUGIN_JSON" ] && command -v python3 &>/dev/null; then
    WB_FIELDS=$(python3 -c "import json; keys=list(json.load(open('${WB_PLUGIN_JSON}')).keys()); print(','.join(keys))" 2>/dev/null || echo "")
fi

if [ -n "$REF_FIELDS" ] && [ -n "$WB_FIELDS" ]; then
    IFS=',' read -ra REF_ARR <<< "$REF_FIELDS"
    IFS=',' read -ra WB_ARR <<< "$WB_FIELDS"
    for f in "${REF_ARR[@]}"; do
        found=false
        for wb in "${WB_ARR[@]}"; do
            [ "$f" = "$wb" ] && found=true && break
        done
        if [ "$found" = false ]; then
            MISSING_FIELDS+="${SEP}\"${f}\""
            SEP="," || true
        fi
    done
...
# Dimension weights: skills 40%, hooks 20%, agents 20%, plugin_fields 20%
SKILL_COV=0
HOOK_COV=0
AGENT_COV=0
FIELD_COV=0
...
if [ "$REF_SKILLS_COUNT" -gt 0 ]; then
    SKILL_COV=$(echo "scale=1; ${WB_SKILLS_COUNT} / ${REF_SKILLS_COUNT} * 100" | bc 2>/dev/null || echo "0")
fi
if [ "$REF_HOOKS_COUNT" -gt 0 ]; then
    HOOK_COV=$(echo "scale=1; ${WB_HOOKS_COUNT} / ${REF_HOOKS_COUNT} * 100" | bc 2>/dev/null || echo "0")
fi
if [ "$REF_AGENTS_COUNT" -gt 0 ]; then
    AGENT_COV=$(echo "scale=1; ${WB_AGENTS_COUNT} / ${REF_AGENTS_COUNT} * 100" | bc 2>/dev/null || echo "0")
...
# Count reference plugin fields
REF_FIELD_COUNT=$(echo "$REF_FIELDS" | tr ',' '\n' | grep -c . || echo "0")
# Count missing: if MISSING_FIELDS is empty, 0; else count commas + 1
if [ -z "$MISSING_FIELDS" ]; then
    MISSING_FIELD_COUNT=0
else
    MISSING_FIELD_COUNT=$(( $(echo "$MISSING_FIELDS" | grep -o ',' | wc -l | tr -d ' ') + 1 ))
fi
if [ "$REF_FIELD_COUNT" -gt 0 ] && [ -n "$MISSING_FIELD_COUNT" ]; then
    PRESENT_FIELDS=$((REF_FIELD_COUNT - MISSING_FIELD_COUNT))
    FIELD_COV=$(echo "scale=1; ${PRESENT_FIELDS} / ${REF_FIELD_COUNT} * 100" | bc 2>/dev/null || echo "0")
fi

# Weighted total
TOTAL_COV=$(echo "scale=1; ${SKILL_COV} * 0.4 + ${HOOK_COV} * 0.2 + ${AGENT_COV} * 0.2 + ${FIELD_COV} * 0.2" | bc 2>/dev/null || echo "0")
TOTAL_COV_INT=$(echo "${TOTAL_COV}" | cut -d'.' -f1 2>/dev/null || echo "0")

# ---- Build JSON ----
MISSING_FIELDS_ARR="[${MISSING_FIELDS}]"
...
json+="\"plugin_fields\":{\"ref_count\":${REF_FIELD_COUNT},\"missing\":${MISSING_FIELDS_ARR},\"coverage_pct\":${FIELD_COV}}"
json+="}}"
