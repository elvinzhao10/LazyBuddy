...
if CODEBUDDY_PLUGIN_ROOT="$FIXTURE" LAZYBUDDY_VERIFY_TIMEOUT_SECONDS=1 LAZYBUDDY_VERIFY_REGRESSION_DEPTH=1 LAZYBUDDY_CHILD_PID="$TMP/child.pid" bash "$FIXTURE/scripts/lazybuddy-verify.sh" >"$TMP/verify.json" 2>"$TMP/verify.stderr"; then
