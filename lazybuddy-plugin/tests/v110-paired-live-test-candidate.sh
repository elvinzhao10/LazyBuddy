...
[ "$(find "$TMP/concurrent-out" -maxdepth 1 -type d -name 'live-test-v1.2.3-*' | wc -l | tr -d ' ')" = 2 ] || fail "concurrent assembly published partial layout"
...
printf 'x' >> "$TMP/digest-buddy/lazybuddy-v1.2.3.tar.gz"
...
rm "$TMP/moved-trae/lazytrae-ai-v1.2.3.tgz"
mkdir "$TMP/moved-out"
...
tampered="$first_path/LazyBuddy/lazybuddy-v1.2.3.tar.gz"
chmod u+w "$tampered"
printf 'x' >> "$tampered"
chmod 0444 "$tampered"
expect_failure final-tamper "LazyBuddy/lazybuddy-v1.2.3.tar.gz" \
