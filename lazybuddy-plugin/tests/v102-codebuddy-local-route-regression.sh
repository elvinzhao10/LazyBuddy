...
elif grep -Fq 'release marketplace must contain lazybuddy 1.2.1 from ./lazybuddy-plugin' "$TMP/nested-source.out"; then
...
if env CODEBUDDY_PLUGIN_ROOT="$RELEASE_ROOT/lazybuddy-plugin" \
    ...
    && grep -Fq 'PASS marketplace version agreement: 1.2.1' "$READINESS_OUTPUT" \
