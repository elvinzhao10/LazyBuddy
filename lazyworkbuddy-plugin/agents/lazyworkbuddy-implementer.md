---
name: lazyworkbuddy-implementer
...
- Capture evidence artifacts to the specified evidence path (e.g., `.lazyworkbuddy/evidence/task-<N>-<slug>.<ext>`).
...
- Manual QA artifact: .lazyworkbuddy/evidence/task-<N>-<slug>.<ext>
...
EVIDENCE_RECORDED: .lazyworkbuddy/evidence/task-<N>-<slug>.<ext>
```

## Handoff format

The implementer is a leaf agent — it does not hand off to other agents. The DoneClaim is consumed by the orchestrator, which routes it to a lazyworkbuddy-verifier for independent adversarial verification before marking the task complete.
