#!/usr/bin/env bash
# lazyworkbuddy-mcp-test.sh — integration test for all 8 MCP servers.
...
#
# Usage: bash lazyworkbuddy-mcp-test.sh [repo-root]
set -euo pipefail

ROOT="${1:-$(cd "$(dirname "$0")/../.." && pwd)}"
PLUGIN="$ROOT/lazyworkbuddy-plugin"
export CWD="$ROOT"
...
echo "=== Lazyworkbuddy MCP integration test (8 servers) ==="
echo ""
...
OUT=$(rpc context-graph '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"symbol_refs","arguments":{"symbol":"lazyworkbuddy","limit":3}}}')
...
OUT=$(rpc code-intel '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"symbols","arguments":{"path":"lazyworkbuddy-plugin/mcp/code-intel/server.py"}}}')
