#!/bin/bash
# lazyworkbuddy-security-check.sh — Secret/credential leak scanner (v0.9)
#
...
#
# Usage: ./scripts/lazyworkbuddy-security-check.sh
...
while IFS= read -r -d '' file; do
    ...
    -not -path '*/dist/*' \
    -not -path '*/build/*' \
    -not -path '*/lazyworkbuddy-security-check.sh' \
    -print0)
