...
# LazyCodex source: reference/lazycodex/plugins/omo/hooks/pre-tool-use-enforcing-unlimited-goal-budget.json
...
if [ "$TOOL_NAME" = "Bash" ]; then
    if echo "$TOOL_INPUT" | grep -qE 'rm\s+-rf?\s+/|rm\s+-rf?\s+\$HOME|rm\s+-rf?\s+~/'; then
        echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Destructive recursive delete denied. ASK per .workbuddy/settings.json — destructive operations require explicit confirmation."}}'
        exit 0
    fi
fi
...
if echo "$TOOL_INPUT" | grep -qE 'git\s+push\s+--force|git\s+reset\s+--hard'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Destructive git operation denied. ASK per .workbuddy/settings.json — requires explicit user confirmation."}}'
    exit 0
fi
...
if echo "$TOOL_INPUT" | grep -qE 'npm\s+publish|pip\s+upload|docker\s+push'; then
    echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"External publish operation denied. ASK per .workbuddy/settings.json — requires approval."}}'
    exit 0
fi
