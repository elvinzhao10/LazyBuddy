...
---
_Adapted from LazyCodex git-master. All semantics preserved. `multi_agent_v1` → WorkBuddy Agent tool; `.omo/` → `.lazyworkbuddy/`; `${PLUGIN_ROOT}` → `${CODEBUDDY_PLUGIN_ROOT}`. Git discipline is platform-agnostic — the mode gate, ground truth, atomic commit, rebase, history, and safety invariants are reproduced verbatim._
