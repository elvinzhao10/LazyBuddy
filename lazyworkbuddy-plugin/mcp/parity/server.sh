...
text = open(os.path.join(cwd, 'docs/lazyworkbuddy-parity-ledger.md')).read()
rows, cat = [], 'unknown'
  ...
  tools/call)
    ...
    case "$TNAME" in
      ...
      update_parity_ledger)
        ...
        printf '\n| %s | v0.8 | %s | %s | %s |\n' "$MN" "$NS" "$NOW" "$NT" >> "$CWD/docs/lazyworkbuddy-parity-ledger.md"
...
import json, re, os
cwd = os.environ.get('CWD', '.')
text = open(os.path.join(cwd, 'docs/lazyworkbuddy-known-gaps.md')).read()
gaps = []
